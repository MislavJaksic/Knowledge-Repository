...
The local users/accounts feature serves two main use-cases:
    ...
    Additional users for a very small team when SSO integration is overkill. The local users don't provide advanced features such as groups, login history etc. So if you need such features it is strongly recommended to use SSO.

Make sure to read about security limitations related to local users in security considerations document
...
There are two ways that SSO can be configured:
    ...
    Existing OIDC provider - use this if you already have an OIDC provider which you are using (e.g. Okta, OneLogin, Auth0, Microsoft, Keycloak), where you manage your users, groups, and memberships.
